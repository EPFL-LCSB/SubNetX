...
from addSubnetEcoli import load_model, load_tmodel, find_target_id
...
def prep_score_table(target, rxn_set, pathway_list):
    ...
    for pathway in tqdm(pathway_list, desc='checking pathways'):
        ...
        # finding the yield 
        model.slim_optimize()
        this_dict['Molar Yield'] = abs(model.objective.value/model.reactions.EX_glc__D_e.flux)
        ...
        this_dict['Number of Targets'] = len(intermediates)
    ...
    tmodel = ChassisModel(raw_tmodel, organism='ecoli', met_lexicon={},
                        rxn_lexicon={}, inplace=True)
    integrate_strategy(tmodel,integ_model, rxn_set, target_rxn)
    tmodel.prepare()
    tmodel.convert()
    # Then we block all the added reactions
    for rxn in rxn_set:
