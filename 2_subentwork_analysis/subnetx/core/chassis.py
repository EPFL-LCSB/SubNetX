...
class ChassisModel(ThermoModel):
    ...
    def add_mets_rxns(self, met_list, rxn_list, rxns,
        ...
        # rxns only include the reactions that should be added in this round and hence, all rxns are in rxn_list 
        mets, rxns_stoich = rxn_parser(rxns, rxn_list)
        self.hetero_rxns += rxns
        ...
        added_rxns_id = {k:v for k,v in rxns_id.items() if rxn_id_prefix in v} # only update it for the added reactions
        for id_, rxn_id in added_rxns_id.items():
            the_reaction = self.reactions.get_by_id(rxn_id)
    ...
    def _reconcile_rxns(self, rxns_id):
    # associate pathway reactions with GEM reactions, if already exist
        lexicon = pd.DataFrame.from_dict(self.rxn_lexicon,  orient='index')
        for hash_ in rxns_id.keys():
            try:
                rxns_id[hash_] = [x for x in lexicon[lexicon[LCSBID]== \
                                              hash_].index][0]
            except IndexError:
                continue
